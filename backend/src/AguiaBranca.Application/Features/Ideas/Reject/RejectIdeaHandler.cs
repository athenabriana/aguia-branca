...
namespace AguiaBranca.Application.Features.Ideas.Reject;

/// <summary>Rejeita com comentário obrigatório (R-03.7). Não gera pontos.</summary>
public sealed class RejectIdeaHandler(
    IValidationService validation,
    IIdeaRepository ideas,
    IdeaResponseFactory responses,
    ICurrentUser currentUser,
    IUnitOfWork unitOfWork,
    IClock clock) : IHandler<RejectIdeaCommand, IdeaResponse>
{
    public async Task<Result<IdeaResponse>> HandleAsync(RejectIdeaCommand request, CancellationToken ct)
    {
        ...
        try
        {
            return await unitOfWork.ExecuteInTransactionAsync<Result<IdeaResponse>>(async token =>
            {
                ...
                idea.Reject(currentUser.Id, request.Comment!, clock.UtcNow);
